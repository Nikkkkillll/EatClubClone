    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const [bannerRes, offersRes, categoriesRes, restaurantsRes] =
                    await Promise.all([
                        axios.get("http://localhost:5009/banner"),
                        axios.get("http://localhost:5009/offers"),
                        axios.get("http://localhost:5009/categories"),
                        axios.get("http://localhost:5009/restaurants")
                    ]);

                setBanner(bannerRes.data[0].banner);
                        ...
                        <div className="offers">
                            {offers.map((offer, i) => (
                                <div className="offer-card" key={i}>
                                    {offer.link ? (
                                        <Link to={offer.link}>
                                            <img src={offer.image} alt={`Offer ${i}`} />
                                        </Link>
                                    ) : (
                                        <img src={offer.image} alt={`Offer ${i}`} />
                                    )}
                ...
                <div className="food-container2">
                    ...
                    <div className="restaurants-container">
                        {restaurants.map((rest, i) => (
                            <div className="membership-cart" key={i}>
                                <img src={rest.image} alt={rest.name} />
                                <div className="membership-containt">
                                    <p className="membership-name">{rest.name}</p>
                                    <p className="membership-tagline">{rest.tagline}</p>
                                    <hr />
                                    <div className="offer-contain">
                                        <img
                                            src="https://eatclub.in/assets/images/offers_teal.svg"
                                            alt="offer"
                                        />
                                        <span className="offer-contain-tagline">{rest.offer}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
